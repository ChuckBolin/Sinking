...
cGameStateStatus::cGameStateStatus(void){
}
...
void cGameStateStatus::initialize(){

}

void cGameStateStatus::activate(){
}

cGameStateObject* cGameStateStatus::update(double timeDifference){
...
void cGameStateStatus::render(Graphics &con){
  ...
  gi = g_Sprite.GetSpriteData(46);//damage control
  ...
  con.Draw2DText("Status state - Press F return.", F_V20, con.GetScreenCenterX() - 150, con.GetScreenCenterY() - 10, 0,255,255);
...
void cGameStateStatus::processEvent(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam){
}

void cGameStateStatus::deactivate(){
  CAudioManager *pAudio = CAudioManager::Instance();
  pAudio->KillAll();
  //g_Audio.KillAll();
}
void cGameStateStatus::resume(){
}
void cGameStateStatus::pause(){
}
void cGameStateStatus::save(){
}

void cGameStateStatus::addTransitionEvent(int event, cGameStateObject* p_Next){
  TRANSITION_EVENT te;
  te.event=event;
  te.p_gso = p_Next;
